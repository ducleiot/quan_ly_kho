...
class ExamController extends Controller
{
    ...
    public function find(Request $request)
    {
        ...
        if (count($data) > 0) {
            ...
            //$current_date = $current_date->modify("+5 minutes");
            //date_add($current_date, new DateInterval("PT5M"));
          //  $cur_time=date("Y-m-d H:i:s");
            $duration='+5 minutes';
            $current_date = date('Y-m-d H:i:s', strtotime($duration, strtotime($current_date)));
    ...
    {
        //$assess_qty = Util::getAssessmentQty($exam_class_id);
        ...
        //var_dump($check_exam);
        if ($check_exam != null && $assessment_check > 0 && $exam_class!=null &&  $exam_class->assessment_quantity > 0) {
            ...
            $test_query = DB::table('mark_student')
               ...
               // ->select('mark_student.answer_id', DB::raw('COUNT(mark_student.answer_id) as count_answer_id'), DB::raw('COUNT(mark_student_details.mark_student_id) as count_mark_student_id'))
                ...
                ->where('mark_student.is_success', '=', 1)
            ...
            // $answer_data_current = DB::table('answers')
            //     ->select('*')
            ...
            //     ->where('answers.student_id', '=', auth()->user()->id)
            //     ->where('mark_student.is_success', '!=', 1)
            //     ->orderBy("mark_student.answer_id", "ASC")
            ...
            ->where('answers.exam_class_id', '=', $exam_class_id)
            ->where('answers.exam_id', '=', $exam_id)
            ->where('mark_student.student_id', '=', auth()->user()->id)
            ->where('answers.student_id', '!=', auth()->user()->id)
            ->where('mark_student.is_success', '!=', 1)
            ->orderBy("mark_student.answer_id", "ASC")
            //->take($assessment_check)
            ...
            $count_1 = count($answer_data_current);
            $answer_data = null;
            $total_data_count = $count_1;

            if ($count_1 >= $assessment_check) {
                for($i=0;$i < $assessment_check; $i++){
                    $answer_data[] = $answer_data_current[$i];
                }                
                $total_data_count = $assessment_check;
                
            }else{
                
                $num_delta = $assessment_check - $count_1;
                $answer_data_tmp = DB::table('answers')
                ->select('*')
                ->where('answers.exam_class_id', '=', $exam_class_id)
                ->where('answers.exam_id', '=', $exam_id)
                ->where('answers.student_id', '!=', auth()->user()->id)
                ...
                ->take($num_delta)
                ->get();
                if(count($answer_data_tmp) == $num_delta){
                    $answer_data = $answer_data_current;
                    $current_date = date(Util::$date_time_format);
                    for($i=0;$i < count($answer_data_tmp); $i++){
                        $answer_data[] = $answer_data_tmp[$i];
                        $save_data_1 = array(
                            "answer_id" => $answer_data_tmp[$i]->id,
            ...
            if ($answer_data != null && count($answer_data) > 0) {
                $all_data = [];
            ...
            if ($assessment_check == 0) {
                ...
                return view('frontend.exam.assess_msg')
                    ->with('exam', $exam)
                    ->with('msg', "Bài kiểm tra đã hết hạn hoặc chưa được phép để đánh giá!")
                ;
               // return abort(404);
            }
        }
    ...
    public function studentMark(Request $request)
    {
        ...
        foreach ($data as $k => $val) {
            ...
            $check_data = DB::table('mark_student')
            ->select("*")
            ...
            if($check_data!=null && count($check_data)==1){
                $obj = $check_data[0];
                if($obj->is_success != 1){
                    $studen_mark_id  = $obj->id;
                }
            }else{
            ...
            if ($studen_mark_id != null) {
                DB::table("mark_student_details")->where('mark_student_id', $studen_mark_id)->delete();
                foreach ($val["data"] as $key => $value) {
                    ...
                    if ($mark_student_detail_id == null) {
                        ...
                        $res = DB::table("mark_student")->where('id', $studen_mark_id)->update(array("is_success"=>1));
                    }
                }
...
//    public function viewEvaluation($id, $answer_question_id)
    public function viewEvaluation(Request $request)
    {
        ...
        $evaluations = DB::table("mark_student")
            ...
            ->get()
        ;
        $teacher_evaluations = DB::table("mark_teacher")
        ->select(
            "mark_teacher_details.answer_question_id",
            "mark_teacher_details.mark",
            "mark_teacher_details.evaluation",
            "users.last_name",
            "users.first_name",
        )
        ->leftJoin('mark_teacher_details', 'mark_teacher_details.mark_teacher_id', '=', 'mark_teacher.id')
        ->leftJoin('users', 'mark_teacher.teacher_id', '=', 'users.id')
        ->where('mark_teacher.answer_id', $id)
        ->where('mark_teacher_details.answer_question_id', $answer_question_id)
        ->get()
    ;

      return view("frontend/exam/view_evaluation")
            ->with("data", $evaluations)
            ->with("teacher_data", $teacher_evaluations)
